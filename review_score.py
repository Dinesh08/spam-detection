...
test="Hurry Hurry Hurry! Click this link to grab the offer! and be Amazed! and get a bonus of 1 lakh"

test = test.translate(str.maketrans('', '', string.punctuation))
test = [word for word in test.split() if word.lower() not in stopwords.words('english')]
words = []
for i in test:
    stemmer = SnowballStemmer("english")
...
review=pd.read_csv("review_with_product.csv")
print(review)
...
print('Average Review Spam Score = ',avg_review_score)